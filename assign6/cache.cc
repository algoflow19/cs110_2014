 ...
 * Function: cacheLock
 * --------------------------
 *  Locks the mutex guarding a potential cache entry.
 *  Hash of request object will be used to acquire the relevant mutex
 */
void HTTPCache::cacheLock(const HTTPRequest& request) {
  size_t hash = hashRequest(request);
  cacheLocks[(hash % MAX_MUTEXES)].lock();
}

/**
 * Function: cacheUnlock
 * ---------------------------
 *  Unlocks the mutex guarding a potential cache entry.
 *  Hash of request object will be used to release the relevant mutex
 */
void HTTPCache::cacheUnlock(const HTTPRequest& request) {
  size_t hash = hashRequest(request);
  cacheLocks[(hash % MAX_MUTEXES)].unlock();
}
