...
/**
 * Used to set forwarded protocol field to the http request header
 */
  void setForwardedProto();
/**
 * Used to set setForwarded field to the http request header
 * Throws HTTP Circular Proxy Chain Exception when detected.
 */
  void setForwardedFor(const std::string& clientIP) throw (HTTPCircularProxyChainException);
 ...
 */
 ...
 private:
  ...
  std::string protocol;
/**
 * State variable that decides the behavior of overloaded << operator
 */
  bool usingProxy;
};
