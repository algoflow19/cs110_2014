...
void BlockHeader::hashBlock(vector<Txn>& txns) {
  // TODO: Step 4
  int bs = sizeof(BlockHeader);
  int ts = sizeof(Txn);
  // Calculating length of data buffer
  int len = bs + (txns.size() * ts);

  // Elected to choose dynamic memory as stack would be overrun
  // if by chance we receive a Txn vector with a huge number of 
  // elements.
  char *data = new char[len];
  // Sanitizing hash before blockheader copy to data buffer
  memset(hash, 0, sizeof(hash));
  memcpy(data, (char *)this, bs);
  int offset = bs;
 
  // Adding Txns to the data buffer
  for (Txn& t: txns) {
    memcpy(data + offset, (char *)&t, ts);
    offset+= ts;
  }

  // Computing hash
  computeHash(data, len, hash);
