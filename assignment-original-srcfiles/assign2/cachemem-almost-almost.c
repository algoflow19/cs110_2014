...
int is_sector_in_protected_cache(int sectornum, void *buf) {
    ...
    for (int index = 0; index < PROTECTED_CACHE_LINES; index++) {
        if ((start[index].sector == sectornum) /*&& (start[index].signature == 1)*/) {
...
int is_sector_in_unprotected_cache(int sectornum, void *buf) {
    ...
    if ((cached_sector->sector == sectornum)/* && (cached_sector->signature == 0xCAFEF00D)*/) {
...
int is_sector_in_other_parts_of_cache(int sectornum, void *buf) {
   cache_line * start = get_unprotected_cache_block(); 
   int max_cache = max_cache_line();
   for (int index = 0; index < max_cache; index++) {
       if ((start[index].sector == sectornum) /*&& (start[index].signature == 0xCAFEF00D)*/) {
           memcpy(buf, (const void *)&start[index].buf, DISKIMG_SECTOR_SIZE);
           return 1;
       }
   }
...
//        return 1;
    if (is_sector_in_unprotected_cache(sectornum, buf) == 1)
        return 1;
/*    if (is_sector_in_other_parts_of_cache(sectornum, buf) == 1)
        return 1; */
    return -1;
}


int save_sector_in_first_available_cache_line(int sectornum, void *buf) {
    cache_line * start = get_unprotected_cache_block();
    int max_cache = max_cache_line();
    for (int index = 0; index < max_cache; index++) {
     /*   if (start[index].signature != 0xCAFEF00D) {
            start[index].signature = 0xCAFEF00D;
...
void save_sector_in_cache(int sectornum, void *buf) {
    cache_line* cached_sector = get_cache_line_for_sector(sectornum);
//    if (cached_sector->signature != 0xCAFEF00D) {
        // Unallocated cache block
    cached_sector->sector = sectornum;
//    cached_sector->signature = 0xCAFEF00D;
    memcpy(&cached_sector->buf, (const void *)buf, DISKIMG_SECTOR_SIZE);
/*    } else {
        if (!cache_is_full) {
            if (save_sector_in_first_available_cache_line(sectornum, buf) == 1)
                return;
            cache_is_full = 1;
        }
        // Cannibalize a taken cache line
        cached_sector->sector = sectornum;
        cached_sector->signature = 0xCAFEF00D;
        memcpy(&cached_sector->buf, (const void *)buf, DISKIMG_SECTOR_SIZE);
    }
